...
#define SMB349_OTHER_CHARGER_CRNTS	0x01
...
#define SMB349_ENABLE_WRITE	      	0x01
#define SMB349_DISABLE_WRITE	    	0x00
...
#define ENABLE_CHARGE		          	0x01

#define ENABLE_PIN_CONTROL        	0x05
...
static int smb349_configure_charger(struct i2c_client *client, int value)
{
  ...
  printk("[SMB349] smb349_configure_charger value=%d\n",value);
	...
	if (value) {
		...
		ret = smb349_write(client, SMB349_CMD_REG, (ret | ((1<<ENABLE_CHARGE))));
		if (ret < 0) {
			dev_err(&client->dev, "%s: err %d\n", __func__, ret);
			goto error;
		}
...
void smb349_power_src_det(void)
{
  ...
  printk("[SMB349] smb349_power_src_det\n");
	...
	wake_lock_timeout(&vbus_wake_lock, HZ/4);
...
static int smb349_configure_init_setting(struct i2c_client *client)
{
	...
	ret = smb349_write(client, SMB349_OTHER_CHARGER_CRNTS/*0x01*/, 0x20); // Pre-Charger Current : 3000mA , Termination Current : 200mA
	if(ret < 0)
	{
		dev_err(&client->dev, "%s() Failed in writing register 0x%02x\n", __func__, SMB349_CMD_REG);
		goto error;
	...
	ret = smb349_write(client, SMB349_VRS_FUNC/*0x2*/, 0xa2); 	// OptiCharge disabled
	if (ret < 0) {
		dev_err(&client->dev, "%s: err %d\n", __func__, ret);
		goto error;
	}	
		
	ret = smb349_read(client, 0x01); 
	if (ret < 0) {
		dev_err(&client->dev, "%s: err %d\n", __func__, ret);
		goto error;
	}
	
	// Pin and Enable contorl , I2C Contorl 0 : disable charger
	...
	ret = smb349_write(client, SMB349_PIN_CTRL, (ret & ~((0x3<<ENABLE_PIN_CONTROL))));
	if (ret < 0) {
		dev_err(&client->dev, "%s: err %d\n", __func__, ret);
		goto error;
	}
printk("[SMB349] smb349_write(client, 0x02, 0xa2) \n");
	ret = smb349_write(client, 0x02, 0xa2);

#ifdef SMB349_DEBUG	
	printk("[SMB349] =========================== \n");
	ret = smb349_read(client, 0x00); 
	printk("[SMB349] 0x00 = 0x%x \n", ret);
	ret = smb349_read(client, 0x01); 
	printk("[SMB349] 0x01 = 0x%x \n", ret);
	ret = smb349_read(client, 0x02); 
	printk("[SMB349] 0x02 = 0x%x \n", ret);
	ret = smb349_read(client, 0x03); 
	printk("[SMB349] 0x03 = 0x%x \n", ret);	
	ret = smb349_read(client, 0x04); 
	printk("[SMB349] 0x04 = 0x%x \n", ret);
	ret = smb349_read(client, 0x05); 
	printk("[SMB349] 0x05 = 0x%x \n", ret);
	ret = smb349_read(client, 0x06); 
	printk("[SMB349] 0x06 = 0x%x \n", ret);
	ret = smb349_read(client, 0x07); 
	printk("[SMB349] 0x07 = 0x%x \n", ret);
	ret = smb349_read(client, 0x09); 
	printk("[SMB349] 0x09 = 0x%x \n", ret);
	ret = smb349_read(client, 0x0a); 
	printk("[SMB349] 0x0a = 0x%x \n", ret);
	ret = smb349_read(client, 0x0b); 
	printk("[SMB349] 0x0b = 0x%x \n", ret);	
	ret = smb349_read(client, 0x0c); 
	printk("[SMB349] 0x0c = 0x%x \n", ret);
	ret = smb349_read(client, 0x0d); 
	printk("[SMB349] 0x0d = 0x%x \n", ret);
	ret = smb349_read(client, 0x0e); 
	printk("[SMB349] 0x0e = 0x%x \n", ret);
	ret = smb349_read(client, 0x10); 
	printk("[SMB349] 0x10 = 0x%x \n", ret);	
	ret = smb349_read(client, 0x11); 
	printk("[SMB349] 0x11 = 0x%x \n", ret);	
	ret = smb349_read(client, 0x12); 
	printk("[SMB349] 0x12 = 0x%x \n", ret);
	ret = smb349_read(client, 0x30); 
	printk("[SMB349] 0x30 = 0x%x \n", ret);
	ret = smb349_read(client, 0x31); 
	printk("[SMB349] 0x31 = 0x%x \n", ret);
	ret = smb349_read(client, 0x33); 
	printk("[SMB349] 0x33 = 0x%x \n", ret);	
	ret = smb349_read(client, 0x35); 
	printk("[SMB349] 0x35 = 0x%x \n", ret);	
	ret = smb349_read(client, 0x36); 
	printk("[SMB349] 0x36 = 0x%x \n", ret);		
	ret = smb349_read(client, 0x37); 
	printk("[SMB349] 0x37 = 0x%x \n", ret);	
	ret = smb349_read(client, 0x38); 
	printk("[SMB349] 0x38 = 0x%x \n", ret);	
	ret = smb349_read(client, 0x39); 
	printk("[SMB349] 0x39 = 0x%x \n", ret);		
	ret = smb349_read(client, 0x3a); 
	printk("[SMB349] 0x3a = 0x%x \n", ret);	
	ret = smb349_read(client, 0x3b); 
	printk("[SMB349] 0x3b = 0x%x \n", ret);	
	ret = smb349_read(client, 0x3c); 
	printk("[SMB349] 0x3c = 0x%x \n", ret);		
	ret = smb349_read(client, 0x3d); 
	printk("[SMB349] 0x3d = 0x%x \n", ret);		
	ret = smb349_read(client, 0x3e); 
	printk("[SMB349] 0x3e = 0x%x \n", ret);		
	ret = smb349_read(client, 0x3f); 
	printk("[SMB349] 0x3f = 0x%x \n", ret);		
	printk("[SMB349] =========================== \n");
#endif
...
//	printk("soc=%d vol=%d\n",smb349_bat_info.bat_info.level,smb349_bat_info.bat_info.batt_vol);
	...
	//temp for fake eoc
	if(smb349_bat_info.bat_info.level >= 98)	{
		smb349_bat_info.bat_info.batt_is_full = 1;
	}
	else if(smb349_bat_info.bat_info.level < 96)	{	
