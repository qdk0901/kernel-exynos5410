...
void s5p_dp_init_analog_param(struct s5p_dp_device *dp)
{
	...
	if (!analog_param) {
		...
		dev_dbg(dp->dev, "Set Preset Analog Param TX_AMP_TUNING_CTL=0x%x, PLL_FILTER_CTL_1=0x%0x\n",
            readl(dp->reg_base + S5P_DP_PLL_FILTER_CTL_1),
            readl(dp->reg_base + S5P_DP_TX_AMP_TUNING_CTL));
	} else {
		int tx_amp;
		...
		writel(reg, dp->reg_base + S5P_DP_TX_AMP_TUNING_CTL);
		dev_dbg(dp->dev, "Set Calculated Analog Param TX_AMP_TUNING_CTL=0x%x, PLL_FILTER_CTL_1=0x%0x\n",
            readl(dp->reg_base + S5P_DP_PLL_FILTER_CTL_1),
            readl(dp->reg_base + S5P_DP_TX_AMP_TUNING_CTL));
	}
}
