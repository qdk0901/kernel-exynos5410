...
struct	thermal_cooling_conf {
	...
	int freq_clip_count;
	int boost_mode;
#ifdef THERMAL_WINDOWS
	enum THR_WINATTR	WinAttr[MAX_TRIP_COUNT];
#endif // endif THERMAL_WINDOWS
};
...
static void exynos_check_tmu_noti_state(int temp, int tmu_type);
...
//		pr_info("tmu temperature state %d to %d \n", tmu_old_state, cur_state);
		blocking_notifier_call_chain(&exynos_tmu_notifier, cur_state, &cur_state);
		tmu_old_state = cur_state;
	}
}
...
#ifdef THERMAL_WINDOWS
enum THR_WINATTR findWinAttr(unsigned int temp) {
	int i = 0;

	for(i = 0 ; i < exynos_sensor_conf.cooling_data.freq_clip_count ; i++) {
		if(exynos_sensor_conf.cooling_data.freq_data[i].temp_level <= temp &&
			exynos_sensor_conf.cooling_data.freq_data[i+1].temp_level > temp) {
			return exynos_sensor_conf.cooling_data.WinAttr[i];
		}
	}
...
void showWinAttr(enum THR_WINATTR attr) {
	switch(attr) {
	case WIN_CRIT:
		printk(">>> WIN_CRIT ");
		break;
...
static int exynos_tmu_read(struct exynos_tmu_data *data)
{
	...
	int alltemp[4] = {0,};
#ifdef THERMAL_WINDOWS
	enum THR_WINATTR	tempAttr = WIN_NONE, tmpAttr = WIN_NONE;
	int tempAvg = 0;
#endif
	...
	for (i = 0; i < EXYNOS_TMU_COUNT; i++) {
		...
		if (temp < min)
			min = temp;
#ifdef THERMAL_WINDOWS
		tmpAttr = findWinAttr(temp);
		if(tmpAttr > tempAttr) tempAttr = tmpAttr;
		tempAvg += temp;
#endif

#ifdef PERCORE_LOGGING
		printk("%d ", temp);
#endif
	}
...
#ifdef THERMAL_WINDOWS
//	showWinAttr(tempAttr);
	switch(tempAttr) {
	case WIN_NONE:
	case WIN_NORMAL:
		return min;
	case WIN_WARN:
		tempAvg = (int)(tempAvg / EXYNOS_TMU_COUNT);
		return tempAvg;
	case WIN_CRIT:
	default:
		return max;
...
#ifdef THERMAL_WINDOWS
	exynos_sensor_conf.cooling_data.WinAttr[0] = WIN_NORMAL; // For active mode (105)
	exynos_sensor_conf.cooling_data.WinAttr[1] = WIN_WARN; // For passive - i mode (110)
	exynos_sensor_conf.cooling_data.WinAttr[2] = WIN_WARN; 	 // For passive - ii mode (112)
	exynos_sensor_conf.cooling_data.WinAttr[3] = WIN_CRIT; 	 // For passive - iii mode (114)
	exynos_sensor_conf.cooling_data.WinAttr[4] = WIN_CRIT; 	 // For hot mode (115)
#endif // endif THERMAL_WINDOWS
