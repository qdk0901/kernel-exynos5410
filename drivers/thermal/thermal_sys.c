...
static int gMaxfreq = 0;
static unsigned long dummy_maxstate = 0;
#endif
					...
					int temp, int trip_temp, int trip)
{
	...
	if (temp >= trip_temp) {
		...
		/* Heating up? */
		if (trend > 0) {
			pr_debug("[TMU] PASSIVE: Trend up, temp=%d\n", temp);
...
#ifdef PERCORE_LOGGING
					gMaxfreq = cdev->ops->get_max_state(cdev, &dummy_maxstate);
#endif
				}
			}
		} else if (trend < 0) { /* Cooling off? */
			pr_debug("[TMU] PASSIVE: Trend down, temp=%d\n", temp);
			list_for_each_entry(instance, &tz->cooling_devices,
					    node) {
				if (instance->trip != trip)
					continue;
