...
int wl_android_wifi_on(struct net_device *dev)
{
	...
	if (!g_wifi_on) {
		...
		printk("%s dhd_dev_reset~\n", __FUNCTION__);
		ret = dhd_dev_reset(dev, FALSE);
		printk("# %s dhd_dev_reset ret:%d\n", __FUNCTION__, ret);
		
		if (ret)
			goto err;

		printk("%s sdioh_start~\n", __FUNCTION__);
		ret = sdioh_start(NULL, 1);
		printk("%s sdioh_start ret:%d\n", __FUNCTION__,ret);
		if (!ret) {
			printk("%s dhd_dev_init_ioctl~\n", __FUNCTION__);

			if (dhd_dev_init_ioctl(dev) < 0) {
				printk("%s dhd_dev_init_ioctl error!\n", __FUNCTION__);
				ret = -EFAULT;
				goto err;
			}
		}
