...
static int apds990x_set_pilt(struct i2c_client *client, int threshold)
{
	...
	//pr_info("[APDS]ps_hysteresis_threshold = %d\n", data->ps_hysteresis_threshold);
...
static void apds990x_change_als_threshold(struct i2c_client *client)
{
	...
	//printk("[APDS9900][change als threshold]luxValue is %d\n ",luxValue);//lt, test
...
static void apds990x_als_polling_work_handler(struct work_struct *work)
{
	...
	//printk("[APDS9900][polling work handle]luxValue is %d\n",luxValue);//lt, test
	input_report_abs(data->input_dev_als, ABS_MISC, luxValue); // report the lux level
...
static irqreturn_t apds990x_interrupt(int vec, void *info)
{
	...
	APDSDBG("[APDS990x]==> apds990x_interrupt\n");
				...
				struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if(val == 1) {
		...
		if (data->enable_ps_sensor==0) {
			...
			apds990x_set_control(client, ps_control); /* 100mA, IR-diode, 1X PGAIN, 1X AGAIN */
        ...
        struct device_attribute *attr, const char *buf, size_t count)
{
    ...
    ret = apds990x_set_pilt(client, val);
...
static int apds990x_init_client(struct i2c_client *client)
{
	...
	if (id == 0x20) {
		...
		APDSDBG("Neither APDS-9901 nor APDS-9901\n");
		return -EIO;
	}
				...
				const struct i2c_device_id *id)
{
	...
	// set gpio GPX1_7 function and request irq
	err = gpio_request(EXYNOS5410_GPX1(7), "LIGHT_INT");
	if (err) {
		printk("APDS990x fail to request irq gpio, return :%d\n", err);
		goto exit_unregister_dev_ps;
	}
	s3c_gpio_cfgpin(EXYNOS5410_GPX1(7), S3C_GPIO_SFN(0x0F));
	s3c_gpio_setpull(EXYNOS5410_GPX1(7), S3C_GPIO_PULL_UP);
	gpio_free(EXYNOS5410_GPX1(7));
