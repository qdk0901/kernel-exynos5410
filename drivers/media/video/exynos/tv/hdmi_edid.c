...
#define GET_PRESET_X_MARGIN(xres)	(u32)((xres*25)/100)
#define GET_PRESET_Y_MARGIN(yres)	(u32)((yres*14)/100)
static bool edid_available_preset(u32 mode_res, u32 preset_res, u32 margin)
{
	if ( (mode_res <= preset_res + margin) && (mode_res >= preset_res - margin) ){
		return true;
	}
...
#ifdef CONFIG_MACH_YAMO5410
		if ( (mode->refresh == preset->refresh) && (mode->vmode == preset->vmode) &&
			edid_available_preset(mode->xres, preset->xres, GET_PRESET_X_MARGIN(preset->xres)) &&
			edid_available_preset(mode->yres, preset->yres, GET_PRESET_Y_MARGIN(preset->yres)) ) {
			return preset;
		}
#endif
	}
