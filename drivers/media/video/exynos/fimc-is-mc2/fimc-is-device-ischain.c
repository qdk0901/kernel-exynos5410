	...
	u32 module, u32 info)
{
  ...
  //mm78.kim -- Rotate 180 dgree for back camera
  if(module == SENSOR_NAME_S5K3H7_SUNNY)
	...
	struct fimc_is_frame *frame)
{
	...
	if (device->sensor->framerate == 120) {
		...
		&device->state)) {
                core->clock.dvfs_skipcnt = DVFS_SKIP_FRAME_NUM;
                fimc_is_set_dvfs(core, device, group->id, DVFS_L0, I2C_L0);
#if defined( USE_S5K3L2_CAMERA) || defined ( USE_S5K3H7_CAMERA )  //##mmkim -- this need to check.
        } else if ((device->module == SENSOR_NAME_S5K3L2 ||device->module == SENSOR_NAME_S5K3H7_SUNNY)&&
		test_bit(FIMC_IS_ISCHAIN_REPROCESSING, \
		&device->state)) {
                core->clock.dvfs_skipcnt = DVFS_SKIP_FRAME_NUM;
                fimc_is_set_dvfs(core, device, group->id, DVFS_L0, I2C_L0);
	...
	if (!core->clock.dvfs_skipcnt) {
		if (!pm_qos_request_active(&device->user_qos)) {
			...
			(device->module == SENSOR_NAME_IMX135 &&
				...
				if (((device->setfile && 0xffff) \
					...
					fimc_is_set_dvfs(core, device, group->id, \
						DVFS_L1_3, I2C_L2);
#ifdef USE_S5K3L2_CAMERA  //##mmkim -- this need to check.
		} else if (device->module == SENSOR_NAME_S5K3L2&&
			!test_bit(FIMC_IS_ISCHAIN_REPROCESSING, \
				&device->state)) {
			if ((device->setfile && 0xffff) == \
				ISS_SUB_SCENARIO_VIDEO)
...
#endif
#ifdef USE_S5K3H7_CAMERA  //##mmkim -- this need to check.
		} else if (device->module == SENSOR_NAME_S5K3H7_SUNNY&&
			!test_bit(FIMC_IS_ISCHAIN_REPROCESSING, \
				&device->state)) {
			if ((device->setfile && 0xffff) == \
				ISS_SUB_SCENARIO_VIDEO)
		...
		} else if (device->module == SENSOR_NAME_S5K3H7_SUNNY_2M) {
			if (((device->setfile && 0xffff) \
				== ISS_SUB_SCENARIO_FRONT_VT1) \
				|| ((device->setfile & 0xffff) == \
				ISS_SUB_SCENARIO_FRONT_VT2))
