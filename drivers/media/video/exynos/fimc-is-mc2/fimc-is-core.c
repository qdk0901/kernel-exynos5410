...
int fimc_is_camera_power(unsigned int power, u32 input)
{
  ...
  struct regulator *camflash_1V8_regulator = NULL;

  printk(" +++ fimc_is_camera_power (%d), input (%d)\n", power, input);

  if(input == 0){ //main_camera
      ...
      if (power) {
      	...
      	regulator_enable(camflash_1V8_regulator);
      } else {
         ...
         if(regulator_is_enabled(camflash_1V8_regulator))
          	regulator_disable(camflash_1V8_regulator);
      }
     udelay(1000);
      ...
      regulator_put(camflash_1V8_regulator);	  
  }
  else{ //front camera
...
int fimc_is_runtime_suspend(struct device *dev)
{
	...
	struct fimc_is_core *core
      ...
      #ifdef USE_TF4_SENSOR	
      struct fimc_is_device_ischain *device = (struct fimc_is_device_ischain*)&core->ischain[0];
      struct fimc_is_enum_sensor *active_sensor = (struct fimc_is_enum_sensor *)device->sensor->active_sensor;
      #endif
	...
	if (core->pdata->cfg_gpio) {
       ...
       fimc_is_camera_power(0, active_sensor->flite_ch);
#endif
