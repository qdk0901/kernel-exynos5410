...
int fimc_is_sensor_probe(struct fimc_is_device_sensor *device, u32 channel)
{
	...
	enum_sensor[SENSOR_NAME_S5K3H7_SUNNY].setfile_name = "setfile_imx135.bin";//"setfile_3h7.bin";
	...
	enum_sensor[SENSOR_NAME_S5K3H7_SUNNY_2M].setfile_name =	"setfile_6b2.bin"; //"setfile_3h7.bin";		
	...
	enum_sensor[SENSOR_NAME_S5K3L2].max_framerate = 30;
    ...
    ext->flash_con.product_name = FLADRV_NAME_NOTHING;
    #else
    ext->flash_con.product_name = FLADRV_NAME_LM3559;
    ext->flash_con.peri_type = SE_I2C;
    ext->flash_con.peri_setting.i2c.channel= SENSOR_CONTROL_I2C2;
    #endif   
	ext->from_con.product_name = FROMDRV_NAME_NOTHING;
	ext->mclk = 0;
	ext->mipi_lane_num = 0;
	...
	struct fimc_is_video_ctx *vctx)
{
	...
	if (core->pdata->cfg_gpio) {
      ...
      fimc_is_camera_power(1, device->flite.channel);
  #endif
