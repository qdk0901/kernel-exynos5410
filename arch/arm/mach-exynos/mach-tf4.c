...
static struct platform_device *tf4_lightsensor_i2c_devices[] __initdata = {
	&gpio_i2c3_dev,
};
...
void __init exynos5_tf4_lightsensor_init(void)
{
	i2c_register_board_info(3, i2c_devs3, ARRAY_SIZE(i2c_devs3));
	platform_add_devices(tf4_lightsensor_i2c_devices, ARRAY_SIZE(tf4_lightsensor_i2c_devices));
}
...
static unsigned int tf4_boot_pd_gpio_table[][3] = {
...
#ifdef CONFIG_EXYNOS5_TF4_LOW_POWER_GPIO
static unsigned int tf4_boot_normal_gpio_table[][4] =
{
	...
	{ EXYNOS5410_GPA2(2),  S3C_GPIO_SFN(0x3),		S3C_GPIO_PULL_NONE, -1}, 
	{ EXYNOS5410_GPA2(3),  S3C_GPIO_SFN(0x3),		S3C_GPIO_PULL_NONE, -1},
...
////////////////////////////////////////////////////////////////////////
	...
	{ EXYNOS5410_GPX0(1),  S3C_GPIO_SFN(0xf),   	S3C_GPIO_PULL_NONE, -1},  //TSP_INT
	{ EXYNOS5410_GPX0(2),  S3C_GPIO_SFN(0xf), 		S3C_GPIO_PULL_NONE, -1},  //PMIC_ONOB
	{ EXYNOS5410_GPX0(3),  S3C_GPIO_SFN(0xf),		S3C_GPIO_PULL_UP, 	-1},    //VOLDOWN_KEY
	{ EXYNOS5410_GPX0(4),  S3C_GPIO_SFN(0xf),		S3C_GPIO_PULL_UP, 	-1},    //VOLUP_KEY
	...
	{ EXYNOS5410_GPX2(7),  S3C_GPIO_INPUT, 			S3C_GPIO_PULL_DOWN, -1},  //TP
...
#ifdef CONFIG_EXYNOS5_TF4_LOW_POWER_GPIO
static void exynos5_tf4_gpio_init(void)
{
	// Power Down GPIO Setting on Booting
	u32 i, gpio;

	set_gpio_pd_reg_dirty();
	for (i = 0; i < ARRAY_SIZE(tf4_boot_pd_gpio_table); i++) {
		gpio = tf4_boot_pd_gpio_table[i][0];
		s5p_gpio_set_pd_cfg(gpio, tf4_boot_pd_gpio_table[i][1]);
		s5p_gpio_set_pd_pull(gpio, tf4_boot_pd_gpio_table[i][2]);
	}

	// Normal GPIO Setting on Booting
	for (i = 0; i < ARRAY_SIZE(tf4_boot_normal_gpio_table); i++){
		gpio = tf4_boot_normal_gpio_table[i][0];
		s3c_gpio_cfgpin(gpio, S3C_GPIO_SFN(tf4_boot_normal_gpio_table[i][1]));
		s3c_gpio_setpull(gpio, tf4_boot_normal_gpio_table[i][2]);
		if(tf4_boot_normal_gpio_table[i][3]!=-1)
			gpio_set_value(tf4_boot_normal_gpio_table[i][0], tf4_boot_normal_gpio_table[i][3]);
 	}	
}
#endif
...
#ifdef CONFIG_EXYNOS5_TF4_LOW_POWER_GPIO
static unsigned int tf4_sleep_alive_gpio_table[][3] =
{
	/*Need check later on TF4 EVT board!!, temp Here. */
...
static void tf4_config_sleep_powerdown_gpio_table(void)
{
#ifdef CONFIG_EXYNOS5_TF4_LOW_POWER_GPIO
	printk("-tf4_config_sleep_powerdown_gpio_table\n ");	
	config_sleep_gpio_table(ARRAY_SIZE(tf4_sleep_gpio_table),
			tf4_sleep_gpio_table); 
#endif
}

static void tf4_config_sleep_normal_gpio_table(void)
{
	/*TO DO. unfinished*/
	int i=0, gpio;

	printk("-config tf4_config_sleep_normal_gpio_table\n ");
...
static void __init smdk5410_machine_init(void)
{
	...
	exynos5_tf4_lightsensor_init();
#endif

	s3c_config_sleep_powerdown_gpio_table = tf4_config_sleep_powerdown_gpio_table;
	s3c_config_sleep_normal_gpio_table = tf4_config_sleep_normal_gpio_table;
