...
#define TF4_DRD0_IDDET_GPIO		EXYNOS5410_GPX2(4)
#define TF4_DRD0_VBUSDET_GPIO	EXYNOS5410_GPX2(3)
#define TF4_USB20_IDDET_GPIO		EXYNOS5410_GPX2(6)
#define TF4_USB20_VBUSDET_GPIO	EXYNOS5410_GPX2(5)
...
static int tf4_drd30_vbus_ctrl(struct platform_device *pdev, int on)
{
	int phy_num = pdev->id;
	unsigned gpio;
	int ret = -EINVAL;
...
static int tf4_drd30_get_id_state(struct platform_device *pdev)
{
	int phy_num = pdev->id;
	unsigned gpio;

	if (phy_num == 0)
		gpio = TF4_DRD0_IDDET_GPIO;
	/*else if (phy_num == 1)
		gpio = tf4_ID1_GPIO;*/
	else
		return -EINVAL;
...
static bool tf4_drd30_get_bsession_valid(struct platform_device *pdev)
{
	int phy_num = pdev->id;
	unsigned gpio;

	if (phy_num == 0)
		gpio = TF4_DRD0_VBUSDET_GPIO;
	/*else if (phy_num == 1)
		gpio = tf4_VBUS1_GPIO;*/
	else
		/*
...
static struct dwc3_exynos_data tf4_drd_pdata __initdata = {
	...
	.vbus_ctrl			= tf4_drd30_vbus_ctrl,
	.get_id_state		= tf4_drd30_get_id_state,
	.get_bses_vld		= tf4_drd30_get_bsession_valid,
...
static void __init __maybe_unused tf4_drd0_init(void)
{
	...
	if (gpio_request_one(TF4_DRD0_IDDET_GPIO, GPIOF_IN, "UDRD3_0_ID")) {
		printk(KERN_ERR "failed to request UDRD3_0_ID\n");
		tf4_drd_pdata.id_irq = -1;
	} else {
		s3c_gpio_cfgpin(TF4_DRD0_IDDET_GPIO, S3C_GPIO_SFN(0xF));
#ifdef CONFIG_MACH_TF4		
		s3c_gpio_setpull(TF4_DRD0_IDDET_GPIO, S3C_GPIO_PULL_UP);
#elif defined (CONFIG_MACH_SMDK5410)
		s3c_gpio_setpull(TF4_DRD0_IDDET_GPIO, S3C_GPIO_PULL_NONE);
#else
#error not correct machine
#endif 
		gpio_free(TF4_DRD0_IDDET_GPIO);

		tf4_drd_pdata.id_irq = gpio_to_irq(TF4_DRD0_IDDET_GPIO);
	}

	if (gpio_request_one(TF4_DRD0_VBUSDET_GPIO, GPIOF_IN, "UDRD3_0_VBUS")) {
		printk(KERN_ERR "failed to request UDRD3_0_VBUS\n");
		tf4_drd_pdata.vbus_irq = -1;
	} else {
		s3c_gpio_cfgpin(TF4_DRD0_VBUSDET_GPIO, S3C_GPIO_SFN(0xF));
		s3c_gpio_setpull(TF4_DRD0_VBUSDET_GPIO, S3C_GPIO_PULL_NONE);
		gpio_free(TF4_DRD0_VBUSDET_GPIO);
		...
		tf4_drd_pdata.vbus_irq = gpio_to_irq(TF4_DRD0_VBUSDET_GPIO);
	}

	tf4_drd_pdata.quirks = 0;
#if !defined(CONFIG_USB_XHCI_EXYNOS)
	tf4_drd_pdata.quirks |= FORCE_RUN_PERIPHERAL;
#endif
#else
	tf4_drd_pdata.id_irq = -1;
	tf4_drd_pdata.vbus_irq = -1;
...
static void __init tf4_usbswitch_init(void)
{
	...
	int err;
		...
		pdata->gpio_host_detect = TF4_USB20_IDDET_GPIO; // EXYNOS5410_GPX2(6);
...
#error not correct MACH
		...
		pdata->gpio_host_vbus = TF4_USB20_VBUSDET_GPIO;
