...
#define YAMO5410_WL_REGON			EXYNOS5410_GPJ0(0)
...
// sckim : call when need to recoginze insertion of sd stack.
...
static struct resource yamo5410_wifi_resources[] = {
	[0] = {
...
static struct platform_device yamo5410_wifi = {
	.name		= "bcmdhd_wlan",
	.id		= -1,
	.num_resources	= ARRAY_SIZE(yamo5410_wifi_resources),
	.resource	= yamo5410_wifi_resources,
	.dev		= {
		.platform_data = &wifi_pdata,
	},
};

static int __init yamo5410_wifi_init(void)
{
	int ret;

	wake_lock_init(&wifi_wake_lock, WAKE_LOCK_SUSPEND, "wifi_ctrl_wake_lock");

	wl_power 	= YAMO5410_WL_REGON;

	ret = platform_device_register(&yamo5410_wifi);
	if (ret)
		goto err1;
...
err2:
	platform_device_unregister(&yamo5410_wifi);
err1:
	return ret;
}

device_initcall(yamo5410_wifi_init);
