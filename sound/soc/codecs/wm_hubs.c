...
if (hubs->dcs_codes_l || hubs->dcs_codes_r) {
	...
	/* HPOUT1L */
	if (reg_l + hubs->dcs_codes_l > 0 &&
		reg_l + hubs->dcs_codes_l < 0xff)
		reg_l += hubs->dcs_codes_l;
	dcs_cfg = reg_l << WM8993_DCS_DAC_WR_VAL_1_SHIFT;

	/* HPOUT1R */
	if (reg_r + hubs->dcs_codes_r > 0 &&
		reg_r + hubs->dcs_codes_r < 0xff)
		reg_r += hubs->dcs_codes_r;
	dcs_cfg |= reg_r;
...
#ifdef CONFIG_YAMO5410_JACK
	hubs->hp_startup_mode = 1;
#endif /*CONFIG_YAMO5410_JACK*/
	switch (event) {
		    ...
		    struct snd_kcontrol *kcontrol, int event)
{
	...
	case SND_SOC_DAPM_POST_PMU:
		...
		snd_soc_write(codec, WM8993_ANALOGUE_HP_0, reg);
#if defined (CONFIG_MACH_TF4) || defined(CONFIG_MACH_YAMO5410)
...
#if defined (CONFIG_SND_SOC_SAMSUNG_TF4_WM1811)||defined(CONFIG_SND_SOC_SAMSUNG_YAMO5410_WM1811)
