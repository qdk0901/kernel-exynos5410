...
static struct snd_soc_dai_driver tf4_ext_dai[] = {
	{
		.name = "tf4-modem",
		.playback = {
			.channels_min = 1,
...
static int tf4_card_late_probe(struct snd_soc_card *card)
{
...
static struct snd_soc_card tf4 = {
	...
	.late_probe = tf4_card_late_probe,
};

static struct platform_device *tf4_snd_device;

static int __devinit tf4_snd_probe(struct platform_device *pdev)
{
	int ret = 0;
	...
	ret = snd_soc_register_dais(&pdev->dev, tf4_ext_dai, 
			ARRAY_SIZE(tf4_ext_dai));
	if (ret) {
		dev_err(&pdev->dev, "snd_soc_register_dais is failed.");
	}

	tf4.dev = &pdev->dev;
	ret = snd_soc_register_card(&tf4);
	if (ret) {
...
static int __devexit tf4_snd_remove(struct platform_device *pdev)
{
	snd_soc_unregister_card(&tf4);
	snd_soc_unregister_dais(&pdev->dev, ARRAY_SIZE(tf4_ext_dai));
...
static struct platform_driver tf4_snd_driver = {
	.driver = {
		.owner = THIS_MODULE,
		.name = "tf4-wm1811",
		.pm = &snd_soc_pm_ops,
	...
	.probe = tf4_snd_probe,
	.remove = __devexit_p(tf4_snd_remove),
};

module_platform_driver(tf4_snd_driver);
